...
struct icacheStruct {
    jcell* contents;  /* Cache contents (used differently in different entries) */
...
int createInlineCacheEntry(jcell* contents, BYTE* originalCode);
...
#define CREATE_CACHE_ENTRY(cellp, ip)           \
        iCacheIndex = createInlineCacheEntry((jcell*)cellp, ip);
