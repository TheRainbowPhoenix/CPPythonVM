...
#define FRAMELOCALS(fp) ((jcell*)(fp) - fp->thisMethod->frameSize)
...
struct stackStruct {
    ...
    jcell     cells[STACKCHUNKSIZE];
};

#define getStackHeight() (getSP() - ((jcell*)getFP()) - SIZEOF_FRAME + 1)
#define setStackHeight(x) setSP(((jcell *)getFP()) + (SIZEOF_FRAME - 1) + x)
...
struct frameStruct {
    ...
    jcell*    previousSp; /* Stores the previous stack pointer */
