...
#define inAnyHeap(ptr) \
     (((jcell *)ptr >= AllHeapStart) && ((jcell *)ptr < AllHeapEnd))

#define inCurrentHeap(ptr) \
      (((jcell *)ptr >= CurrentHeap) && ((jcell *)ptr < CurrentHeapEnd))
...
#define SIZE(n)         (((jcell)(n)) >> TYPEBITS)
...
extern jcell* AllHeapStart;   /* Lower limits of any heap space */
extern jcell* AllHeapEnd;

extern jcell* CurrentHeap;    /* Current limits of heap space */
extern jcell* CurrentHeapEnd; /* Current heap top */
...
jcell* callocPermanentObject(long size);
...
unsigned long  getObjectSize(jcell* object);
GCT_ObjectType getObjectType(jcell* object);
...
void makeGlobalRoot(jcell** object);
...
#define IS_TEMPORARY_ROOT(_var_, _value_)                                \
  _var_ = (VERIFY_INSIDE_TEMPORARY_ROOTS                                 \
        ...
        TemporaryRoots[TemporaryRootsLength++].cellp = (jcell *)&_var_,   \
        _var_)
...
#define IS_TEMPORARY_ROOT_FROM_BASE(_var_, _value_, _base_)              \
     _var_ = (VERIFY_INSIDE_TEMPORARY_ROOTS                              \
       ...
       TemporaryRoots[TemporaryRootsLength+1].cellpp = (jcell **)&_var_,  \
       TemporaryRoots[TemporaryRootsLength+2].cellp = (jcell *)_base_,    \
